...
for i in range(len(images)-4):
         for j in range(i+1, len(images)-3):
            status, result = stitcher.stitch([images[i], images[j], images[j+1], images[j+2], images[j+3]])

            if status == 0:
                stitched_image = result
                h, w = stitched_image.shape[:2]
                input_image = input_image[20:h-20, 20:w-20]
